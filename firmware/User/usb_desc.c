...
const uint8_t* usb_desc_get_config_desc()
{
    return MyCfgDescrCDC;
}

uint16_t usb_desc_get_config_desc_len()
{
    const uint8_t* config = usb_desc_get_config_desc();
    return ((uint16_t)(config[2]) + (uint16_t)(config[3] << 8));
}

void usb_desc_set_serial_number(const char* serial)
{
    memset(MySerNumInfo, 0, sizeof(MySerNumInfo));
    int serial_len = MIN(strlen(serial), (sizeof(MySerNumInfo) - 2)/2);
    MySerNumInfo[0] = 2 + serial_len * 2;
    MySerNumInfo[1] = 3;
    for (int i = 0; i < serial_len; i++)
    {
        MySerNumInfo[2 + i * 2] = (uint8_t)serial[i];
    }
}
